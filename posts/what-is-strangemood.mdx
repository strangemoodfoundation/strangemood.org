...
A blockchain is probably not the _only_ way to do this, but it’s the most reasonable. Whoever controls the network and its treasury is in the position to fleece game developers. So instead, Strangemood gives the network and the treasury to a program that no single entity controls, on a blockchain that no single entity controls.

## Who makes Strangemood?

Strangemood is made by Rebase, a little startup you’ve probably never heard of.

Initially Rebase controls 100% of votes in the co-op. As folks sell games on Strangemood, the program automatically creates new votes and gives them to game developers, until eventually game developers own the majority stake of the network.
